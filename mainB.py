...
from treinoTeste import testar, treinar, TestarGrupos, TestarGrupos_Minimo, TestarGrupos_F1_Macro
...
from downDados import pegar_dados_iid
...
for args.comDefesa in [True, False]:
    ...
    for wm in [0, 4, 10]:
        ...
        testGlobal, testSet, trainSetList, classes = pegar_dados_iid() # pega dados
        ...
        for epoca in range(args.epoca):
            ...
            for i in range(args.num_cliente):
                ...
                if istr in votos and votos[istr]:
                    ...
                    acc, asr, _, _, _ = testar(modeloVencedor, testGlobal, device, classes, args, fazerMatriz=False)

                    asrtotal += asr/args.num_cliente
                    acctotal += acc/args.num_cliente

            historico_acc.append(acctotal)
            historico_asr.append(asrtotal)
        ...
        plt.title(f'performace: {cenario}({wm} atacantes)\nData Poisoning Puro')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.ylim(-5, 105)
        nome_arquivo = f"resultado_{cenario}_{wm}_atk.png"
        plt.savefig(os.path.join(pasta_execucao_atual))
        plt.close()
